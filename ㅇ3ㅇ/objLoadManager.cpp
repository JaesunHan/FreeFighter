...
vector<gameObject> objLoadManager::load(const WCHAR * folder, const WCHAR * fileName, D3DXVECTOR3 * half)
{
	vector<gameObject> objects;
	gameObject* tempObject = NULL;
	wstring fullPath = _T(".\\") + wstring(folder) + _T("\\") + wstring(fileName);
	...
	while (!fin.eof())
	{
		...
		else if (str[0] == 'm')
		{
			...
			MultiByteToWideChar(CP_ACP, 0, mtlFileName, -1, tempFileName, sizeof(tempFileName));

			if (mtlFileName[0] == '.' && mtlFileName[1] == '/')
				mtls = loadMtlLib(folder, &tempFileName[2]);
			else
				mtls = loadMtlLib(folder, &tempFileName[0]);
		}
		else if (str[0] == 'u')
		{
			sscanf_s(str, "usemtl %s", mtlName, sizeof(mtlName));
...
map<string, tagObjectMtlData> objLoadManager::loadMtlLib(const WCHAR * folder, const WCHAR * fileName)
{
	wstring realFilePath = _T(".\\") + wstring(folder) + _T("\\") + wstring(fileName);
	fstream fin;
	fin.open(realFilePath.c_str());
